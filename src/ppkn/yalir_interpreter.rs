...
impl<'a> Interpreter<'a> {
	pub fn new(program: &'a Program) -> Self {
		...
		Self { program, memory, strings: Strings::new() }
	}
	...
	pub fn interpret(&mut self, function: &str) -> Result<Vec<Value>, (Rc<str>, String)> {
		...
		loop {
			...
			if DEBUG_LOGGING {
				println!("{:X?}", stack);
				println!("{:2?}: {:02X?}", current_instr, NAMES[f.code[current_instr] as usize]);
			}
			match f.code[current_instr] {
				...
				RETURN => todo!(),
				CALL_FUNC => {
					let callee = u32::from_ne_bytes(f.code[current_instr + 1..current_instr + 5].try_into().unwrap()) as usize;
					current_instr += 4;
				...
				MEMORY_GROW => todo!(),
				I32_CONST => {
					let x = i32::from_ne_bytes(f.code[current_instr + 1..current_instr + 5].try_into().unwrap());
					current_instr += 4;
					stack.push(x as u64);
				}
				F32_ABS => {
	...
	fn call_import(&mut self, stack: &mut Vec<u64>, callee: usize) {
		...
		match (&callee.namespace[..], &callee.func_name[..]) {
			("std", "string") => {
				...
				let length = u32::from_le_bytes(self.memory[ptr + 4..ptr + 8].try_into().unwrap()) as usize;
				let string = String::from_utf8(self.memory[ptr + 8..ptr + 8 + length].to_owned()).unwrap().into_boxed_str();
		...
		debug_assert_eq!(stack_size - callee.signature.parameters.len() + callee.signature.result.len(), stack.len());
	}
}

impl Strings {
	pub fn new() -> Self {
		Self { strings: HashMap::new(), next_id: 0xABC0 }
	}
