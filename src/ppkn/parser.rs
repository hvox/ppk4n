...
impl<'a> Parser<'a> {
    ...
    fn block(&mut self) -> Result<Block<'a>, SyntaxError<'a>> {
        self.call_counter
            .entry(("block", self.position))
            .and_modify(|x| *x += 1)
            .or_insert(1);
        if DEBUG_LOGGING {
            eprintln!(
                "Parsing block at {:?}",
                &self.tokens[self.position..self.tokens.len().min(self.position + 4)]
            );
        }
    ...
    fn statement(&mut self) -> Result<Expr<'a>, SyntaxError<'a>> {
        ...
        if let Ok(expr) = self.print() {
            ...
            Ok(Expr::new(
                span(start, value.source),
                Definition(variable, typ, value),
            ))
    ...
    fn definition(
        ...
        self.call_counter
            .entry(("definition", self.position))
            .and_modify(|x| *x += 1)
            .or_insert(1);
        if DEBUG_LOGGING {
            eprintln!(
                "Parsing definition at {:?}",
                &self.tokens[self.position..self.tokens.len().min(self.position + 4)]
            );
        }
    ...
    fn assignment(&mut self) -> Result<(Identifier<'a>, Box<Expr<'a>>), SyntaxError<'a>> {
        self.call_counter
            .entry(("assignment", self.position))
            .and_modify(|x| *x += 1)
            .or_insert(1);
        let start_position = self.position;
        let variable = self.expect_identifier()?;
    ...
    fn while_loop(&mut self) -> Result<Expr<'a>, SyntaxError<'a>> {
        self.call_counter
            .entry(("while_loop", self.position))
            .and_modify(|x| *x += 1)
            .or_insert(1);
        let start_position = self.position;
    ...
    fn function(&mut self) -> Result<Expr<'a>, SyntaxError<'a>> {
        ...
        Ok(Expr::new(
            ...
            ExprKind::Function(name.source, args, result, body),
        ))
    }
    ...
    fn expression(&mut self) -> Result<Expr<'a>, SyntaxError<'a>> {
        self.call_counter
            .entry(("expr", self.position))
            .and_modify(|x| *x += 1)
            .or_insert(1);
        use TokenKind::*;
        let mut expr = self.arithmetic_expression()?;
        if let Ok(op) = self.expect_any(&[
            Less,
            LessEqual,
            EqualEqual,
            GreaterEqual,
    ...
    fn expect_typename(&mut self) -> Result<&'a str, SyntaxError<'a>> {
        ...
        Ok(span(
            self.tokens[start_position].source,
            self.tokens[self.position - 1].source,
        ))
    }
    ...
    fn expect_any(&mut self, expected_tokens: &[TokenKind]) -> Result<Token<'a>, SyntaxError<'a>> {
        ...
        self.error(Box::leak(Box::new(format!(
            "Expected {:?}",
            expected_tokens
        ))))
    }
