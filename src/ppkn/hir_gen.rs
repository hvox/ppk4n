...
impl Typechecker {
    ...
    fn typecheck(mut self, ast: &Block<()>) -> hir::Program {
        const TYPECHECKED_FUNCTION_BODY_PLACEHOLDER: hir::Expr<Type> = hir::Expr {
        ...
        for (fn_idx, body) in bodies.into_iter().enumerate() {
            ...
            let function_body = hir::Expr {
                kind: hir::ExprKind::Block(self.typecheck_block(body, self.restype)),
                typ: self.restype,
            };
