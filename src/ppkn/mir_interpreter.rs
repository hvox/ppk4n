    ...
    fn eval_unit(&self, locals: &mut [u64], instr: &InstrCntrl) -> Result<(), Interruption<'a>> {
        match &*instr.kind {
            InstrKindCntrl::Call(f, params) => {
                ...
                self.eval_fn(*f, args)
                    .map_err(|err| Interruption { result: Err(err) })?;
            }
            ...
            InstrKindCntrl::DefVec(id, value, typ) => {
                locals[*id] = self.eval_vec(locals, value, typ)?
            }
            ...
            InstrKindCntrl::Return(instr) => {
                return Err(Interruption {
                    result: Ok(self.eval_instr(locals, instr)?),
                });
            }
    ...
    fn eval_str(&self, locals: &mut [u64], instr: &InstrStr) -> Result<Str, Interruption<'a>> {
        ...
        Ok(match &*instr.kind {
            ...
            CastVec(instr_vec, typ) => match typ {
                ...
                Type::I64 => {
                    let vec = unsafe {
                        Rc::from_raw(self.eval_vec(locals, instr_vec, typ)? as *const Vec<u64>)
                    };
                    let s = format!("{:?}", vec);
                    Rc::into_raw(vec);
                    s.into()
