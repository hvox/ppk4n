...
impl<'a, T> ExprKind<'a, T> {
        ...
        F: FnMut(T, &ExprKind<'a, R>) -> R,
    {
        ...
        match self {
            ...
            If(expr, then, otherwise) => If(
                ...
                Block {
                    ...
                    stmts: then
                        .stmts
                        .into_iter()
                        .map(|stmt| stmt.map_annotations(f))
                        .collect(),
                },
            ...
            Binary(lhs, op, rhs) => Binary(
                Box::new(lhs.map_annotations(f)),
                op,
                Box::new(rhs.map_annotations(f)),
            ),
        ...
        F: FnMut(T, &'a str, &ExprKind<'a, R>) -> Result<R, E>,
    {
        ...
        Ok(match self {
            ...
            Definition(var, typ, expr) => {
                Definition(var, typ, Box::new(expr.try_map_annotations(f)?))
            }
            ...
            Binary(lhs, op, rhs) => Binary(
                Box::new(lhs.try_map_annotations(f)?),
                op,
                Box::new(rhs.try_map_annotations(f)?),
            ),
            FunctionCall(name, args) => {
                FunctionCall(name, try_map(args, |arg| arg.try_map_annotations(f))?)
            }
            MethodCall(obj, func, args) => MethodCall(
                obj.try_map_annotations(f)?.into(),
                func,
                try_map(args, |arg| arg.try_map_annotations(f))?,
            ),
            Variable(x) => Variable(x),
            Integer(x) => Integer(x),
            String(x) => String(x),
