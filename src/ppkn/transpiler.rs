...
impl<'a, 'b> FunctionTranspiler<'a, 'b> {
    ...
    fn transpile_all(&mut self, vec: &Vec<Instr>) -> String {
        vec.into_iter()
            .map(|instr| self.transpile_instr(instr))
            .collect::<Vec<_>>()
            .join(", ")
    }
    ...
    fn transpile_unit(&mut self, instr: &InstrCntrl) -> String {
        ...
        match &*instr.kind {
            ...
            SetI64(var, instr) => {
                format!("{} = {}", self.f.locals[*var].0, self.transpile_i64(instr))
            }
            SetU64(var, instr) => {
                format!("{} = {}", self.f.locals[*var].0, self.transpile_u64(instr))
            }
            SetF64(var, instr) => {
                format!("{} = {}", self.f.locals[*var].0, self.transpile_f64(instr))
            }
            SetStr(var, instr) => {
                format!("{} = {}", self.f.locals[*var].0, self.transpile_str(instr))
            }
            SetBool(_, _) => todo!(),
            SetVec(_) => todo!(),
            PrintStr(instr) => match &*instr.kind {
                InstrKindStr::CastI64(instr) => {
                    format!("print({}, end=\"\")", self.transpile_i64(instr))
                }
                InstrKindStr::CastF64(instr) => {
                    format!("print({}, end=\"\")", self.transpile_f64(instr))
                }
                InstrKindStr::CastVec(instr, _) => {
                    format!("print({}, end=\"\")", self.transpile_vec(instr))
                }
