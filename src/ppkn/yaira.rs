...
impl<T> CodeKind<T> {
    pub fn map<R>(self, f: &mut impl FnMut(T) -> R) -> CodeKind<R> {
        match self {
            ...
            CodeKind::Block(stmts) => {
                CodeKind::Block(stmts.into_iter().map(|stmt| stmt.map(f)).collect())
            }
            ...
            CodeKind::FnCall(name, args) => {
                CodeKind::FnCall(name, args.into_iter().map(|arg| arg.map(f)).collect())
            }
            CodeKind::MethodCall(name, object, args) => CodeKind::MethodCall(
                name,
                object.map(f).into(),
            ...
            CodeKind::While(condition, body) => {
                CodeKind::While(condition.map(f).into(), body.map(f).into())
            }
