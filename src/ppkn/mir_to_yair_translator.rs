			...
			functions.insert(
				...
				yair::Function {
					...
					params: f
						.params
						.into_iter()
						.map(|(name, typ)| yair::Variable { name: name.clone(), typ: translate_type(typ) })
						.collect(),
					result: yair::Variable { name: "result".into(), typ: translate_type(f.result) },
					body: translate_body(f.body),
				},
			);
		}
		yair::Program { types: IndexMap::new(), fns: functions }
	}
}
...
fn translate_body(mir_stmts: Vec<mir::InstrCntrl>) -> yair::Expr {
	...
	yair::Expr { kind: yair::ExprKind::Block(stmts), typ: "".into() }
}
...
fn translate_type(typ: mir::Type) -> yair::Type {
	match typ {
		...
		mir::Type::Vec(typ) => {
			...
			yair::Type { path: "[]".into(), args: vec![translate_type(typ)] }
		}
	}
}
