...
impl Typechecker {
    ...
    fn typecheck(mut self, ast: &Block<()>) -> Program {
        const TYPECHECKED_FUNCTION_BODY_PLACEHOLDER: Code<Type> = Code {
            kind: CodeKind::Unreachable,
            typ: Type::Unit,
        };
        ...
        for (fn_idx, body) in bodies.into_iter().enumerate() {
            ...
            let body = Expr {
                source: body.source,
                kind: ExprKind::Indented(body.clone()),
                annotations: (),
            };
    ...
    fn typecheck_expr(&mut self, expr: &Expr<()>, expected_type: TypeId) -> Code<TypeId> {
        ...
        let kind = match &expr.kind {
            ExprKind::Print(expr) => {
                ...
                CodeKind::FnCall(
                    Str::from("print"),
                    vec![self.typecheck_expr(expr, self.types.str)],
                )
            }
