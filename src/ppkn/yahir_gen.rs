...
impl Typechecker {
	fn new() -> Self {
		let mut typechecker =
			Self { fns: IndexMap::new(), types: Types::new(), restype: TypeId(0), scope: vec![], std_api: HashMap::new() };
		...
		for line in api_tsv.lines().skip(1) {
			...
			let method = row[1];
			let params = row[2].split(", ").filter(|&x| x != "").map(|typ| typechecker.parse_unchecked_type(typ)).collect();
		...
		for Expr { source, kind, annotations } in &ast.stmts {
			match kind {
				ExprKind::Function(name, parameters, result, body) => {
					...
					self.fns.insert(
						name.clone(),
						Function { name, signature, parameters: param_names, body: TYPECHECKED_FUNCTION_BODY_PLACEHOLDER },
					);
				}
				// TODO: return error
