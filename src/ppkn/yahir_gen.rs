...
impl Typechecker {
    fn new() -> Self {
        ...
        for line in api_tsv.lines().skip(1) {
            ...
            let params = row[2]
                .split(", ")
                .filter(|&x| x != "")
                .map(|typ| typechecker.parse_unchecked_type(typ))
                .collect();
            let result = typechecker.parse_unchecked_type(row[3]);
            typechecker
                .std_api
                .insert((self_type, method), (params, result));
        }
        typechecker
    }

    fn typecheck(mut self, ast: &Block<()>) -> yahir::Program {
        const TYPECHECKED_FUNCTION_BODY_PLACEHOLDER: yahir::Expr<Type> = yahir::Expr {
            kind: yahir::ExprKind::Unreachable,
            typ: Type::Unit,
        };
        ...
        for (fn_idx, body) in bodies.into_iter().enumerate() {
            ...
            self.scope[0].clear();
            for (param, typ) in self.fns[fn_idx]
                .parameters
                .iter()
                .zip(&self.fns[fn_idx].signature.params)
            {
            ...
            let function_body = yahir::Expr {
                kind: yahir::ExprKind::Block(self.typecheck_block(body, self.restype)),
                typ: self.restype,
            };
        ...
        yahir::Program {
            fns: self.fns,
            exports: IndexSet::from(["main".into()]),
        }
    }
    ...
    fn typecheck_block(
        ...
        if block.stmts.is_empty() {
            ...
            final_operands = if self.types[expected_type] != UncheckedType::Unit {
                1
            } else {
                0
            };
        };
        let stmts: Vec<_> = block.stmts[0..block.stmts.len() - final_operands]
            .iter()
    ...
    fn typecheck_stmt(&mut self, stmt: &Expr<()>) -> yahir::Stmt<TypeId> {
        match &stmt.kind {
            ...
            ExprKind::If(_, _, _) | ExprKind::Indented(_) => {
                yahir::Stmt::Expr(self.typecheck_expr(stmt, self.types.unit))
            }
            _ => {
    ...
    fn typecheck_expr(&mut self, expr: &Expr<()>, expected_type: TypeId) -> yahir::Expr<TypeId> {
        ...
        let kind = match &expr.kind {
            ExprKind::Print(expr) => {
                self.types
                    .unify(expected_type, self.types.unit)
                    .expect("Type Error");
                let mut args = vec![];
                let expected_type = self.types.add(UncheckedType::Unknown);
            ...
            ExprKind::If(condition, then, otherwise) => {
                ...
                if self.types[expected_type] == UncheckedType::Unit {
                    ...
                    let otherwise = if let Some(otherwise) = otherwise {
                        Some(Box::new(self.typecheck_expr(
                            &otherwise.clone().into_expr(),
                            expected_type,
                        )))
                    } else {
                        self.types.unify(expected_type, self.types.unit);
    ...
    fn resolve_method(&mut self, self_type_id: TypeId, method_name: &str) -> (Vec<TypeId>, TypeId) {
        ...
        if let Some((params, result)) = self.std_api.get(&(self_type, method_name)) {
            if DEBUG_LOGGING {
                println!(
                    "Found method {:?}.{}({:?}) -> {:?}",
                    self_type, method_name, params, result
                );
            }
            let params = params
                .iter()
                ...
                .collect();
            let result = if result == &self_type {
                self_type_id
            } else {
    ...
    fn local(&self, variable: &str) -> TypeId {
        for scope in self.scope.iter().rev() {
            let Some(&type_id) = scope.get(variable) else {
                continue;
            };
            return type_id;
        }
