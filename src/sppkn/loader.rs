...
pub fn load(root: PathBuf, name: Str, source: Str) -> Loader {
	let mut loader = Loader { root_path: root, sources: HashMap::new(), modules: IndexMap::new(), errors: Vec::new() };
...
impl Loader {
	fn load(&mut self, name: Str) -> io::Result<()> {
		...
		self.errors.extend(errors.into_iter().map(|error| error.into_error(name.clone())));
		self.modules.insert(name.clone(), ast);
		for Dependency { location, name: dependency_name } in deps {
			if let Err(_) = self.load(dependency_name) {
				self.errors.push(Error::new(name.clone(), location, PpknErrorKind::LoadError, "Failed to load module".into()));
			}
		}
		return Ok(());
	}
